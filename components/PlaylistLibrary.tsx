...
export default function PlaylistLibrary() {
  ...
  return <div className="library-grid">{tracks.map((track, i) => <article className="library-row" key={`${track.videoId}-${i}`}><span className="library-number">{String(i + 1).padStart(2, "0")}</span><div className="library-art">{track.thumbnail && <img src={track.thumbnail} alt="" />}</div><div className="library-copy"><h3>{track.title}</h3><p>{track.artist}</p></div><span className="library-duration">{track.duration ? `${Math.floor(track.duration / 60)}:${String(track.duration % 60).padStart(2, "0")}` : ""}</span></article>)}</div>;
}
